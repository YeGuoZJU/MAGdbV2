...
list.files()
tree <- read.tree("all_bac_tree.nwk")

# 根据支持度排序，选择前1000个支持度最高的节点
sorted_nodes <- order(as.numeric(tree$node.label), decreasing = TRUE)  # 按支持度排序
top_nodes <- sorted_nodes[1:1000]  # 选择前1000个节点

# 保留前1000个支持度最高的节点
tree <- drop.tip(tree, tree$tip.label[-top_nodes])

# 输出处理后的树到文件
write.tree(tree, file = "1000_filtered_tree.nwk")

# 取出树的标签（Bin）
tree_labels <- tree$tip.label

write.table(tree$tip.label, file = "1000_tip_labels.csv", sep = ",", quote = FALSE, row.names = FALSE, col.names = FALSE)
...
write.table(filtered_annotations, file = "1000_tree_labels.csv", sep = ",", quote = FALSE, row.names = F, col.names = T)

# 手动处理，将门数量较少的统一称为others,重新命名文件为1000_tree_labels.txt
data_file_1 <- "1000_tree_labels.txt"
...
# 设置颜色映射
pal.alb <- c("others" = "#B7796E",
             "p__Bacillota" = "#BCD7E3",
             "p__Bacillota_A" = "#99CFED",
             "p__Bacillota_C" = "#82A6C1",
             "p__Chloroflexota" = "#7778AE",
             "p__Cyanobacteriota" = "#88CAB9",
             "p__Firmicutes" = "#E2AAC1",
             "p__Firmicutes_A" = "#9ABB87",
             "p__Firmicutes_C"= "#EC9C46",
             "p__Fusobacteriota" ="#E8E49F")
...
legend.labels <- list("others" = "others",
             ...
             "p__Chloroflexota" = "p__Chloroflexota",
             "p__Cyanobacteriota" = "p__Cyanobacteriota",
             "p__Firmicutes" = "p__Firmicutes",
             "p__Firmicutes_A" = "p__Firmicutes_A",
             "p__Firmicutes_C"= "p__Firmicutes_C",
             "p__Fusobacteriota" ="p__Fusobacteriota")
